...
is( $space->name,                   'DCIP3LINEAR', 'color space has name: "DCIP3LINEAR"');
is( $space->name('alias'),          'LINEARDCIP3', 'color space has alias name "LINEARDCIP3"');
is( $space->is_name('dci-p3-linear'),           1, 'original name works too');
is( $space->is_name('DCI P3 Linear'),           1, 'even "DCI P3 Linear" works');
is( $space->is_name('Linear-DCI P3'),           1, 'another way to write the alias name');
is( $space->is_name('RGB'),                     0, 'it is not SRGB');
is( $space->is_axis_name('DCIP3LINEAR'),        0,      'space name is not axis name');
...
is( $space->axis_count,                         3, 'DCI P3 Linear color space has 3 axis');
is( $space->is_euclidean,                       1, 'DCI P3 Linear is euclidean');
is( $space->is_cylindrical,                     0, 'DCI P3 Linear is not cylindrical');
...
is_tuple( $rgb, [0.2, 0.3, 0.7], [qw/red green blue/], 'found right values in CSS string');
