...
is( ref $def->range_check([0,0,0]),       'ARRAY', 'check RGB values works on lower bound values');
is( ref $def->range_check([255,255,255]), 'ARRAY', 'check RGB values works on upper bound values');
is( ref $def->range_check([0,0]),              '', "RGB got too few values");
is( ref $def->range_check([0, 0, 0, 0]),       '', "RGB got too many values");
is( ref $def->range_check([-1, 0, 0]),         '', "red value is too small");
is( ref $def->range_check([0.5, 0, 0]),        '', "red value is not integer");
is( ref $def->range_check([256, 0, 0]),        '', "red value is too big");
is( ref $def->range_check([0, -1, 0]),         '', "green value is too small");
is( ref $def->range_check([0, 0.5, 0]),        '', "green value is not integer");
is( ref $def->range_check([0, 256, 0]),        '', "green value is too big");
is( ref $def->range_check([0, 0, -1 ] ),       '', "blue value is too small");
is( ref $def->range_check([0, 0, 0.5] ),       '', "blue value is not integer");
is( ref $def->range_check([0, 0, 256] ),       '', "blue value is too big");
