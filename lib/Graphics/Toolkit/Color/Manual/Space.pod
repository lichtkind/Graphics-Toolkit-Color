...
GTC supports 33 color spaces. This page lists them all and displays their
name, alias name, family name, axis names (long and short and their role)
as well as their default value ranges.
Some relevant context knowledge and background information is also added.
See a condensed overview om the L<color spaces|Graphics::Toolkit::Color::Manual::CheatSheert>.
...
more similar they are in that property.
All color spaces are finite and only certain value ranges along an
axis are acceptable. Most spaces have 3 dimensions (axes) and are
completely linear like in Euclidean (everyday) geometry.
A few spaces have more axes and some spaces are cylindrical. That
means that some axes are not lines but circles and the associated value
describes an angle (the axis is angular).
...
the I<chroma> axis of the I<LCH> space gets the role name I<saturation> 
so GTC can treat the I<chroma> value as synonym of the I<HSL> I<saturation>
value. Not every color space belongs to a family. The spaces on this page
...
(Alias name is B<SRGB> - standard RGB, I<RGB> family) is the default color space
of this module.
...
It is a completely Cartesian (Euclidean) 3D space and thus an RGB tuple 
...
the mix that is displayed on a black background, so that black is (0, 0, 0), 
white (255, 255, 255) and a pure red (fully saturated color) is (255, 0, 0). 
RGB has a gamma of 2.4, which is clipped in the corners (piecewise transfer function).
