...
is( ref $space->check_range([0, 0, 0]),     'ARRAY', 'check HSB values works on lower bound values');
is( ref $space->check_range([360,100,100]), 'ARRAY', 'check HSB values works on upper bound values');
is( ref $space->check_range([0,0]),              '', "HSB got too few values");
is( ref $space->check_range([0, 0, 0, 0]),       '', "HSB got too many values");
...
is( ref $space->check_range([0, 0, -1 ] ),       '', "brightness value is too small");
is( ref $space->check_range([0, 0, 1.1] ),       '', "brightness value is not integer");
is( ref $space->check_range([0, 0, 101] ),       '', "brightness value is too big");
