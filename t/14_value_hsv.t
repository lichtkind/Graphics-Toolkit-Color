...
my @hsv = $def->clamp();
is( int @hsv,  3,     'clamp all three missing values to zero');
...
is( $hsv[0],   1,     'carried first value');
is( $hsv[1],   2,     'carried second value');
is( $hsv[2],   0,     'set missing color value to zero (V)');
@hsv = $def->clamp(1,2,3,4);
...
@hsv = $def->clamp(-1,-1,-1);
is( int @hsv,  3,     'color is clamped up');
is( $hsv[0], 359,     'too low hue value is rotated up');
is( $hsv[1],   0,     'too low green value is clamped up');
is( $hsv[2],   0,     'too low blue value is clamped up');
@hsv = $def->clamp(360, 101, 101);
is( int @hsv,  3,     'color is clamped up');
is( $hsv[0],   0,     'too high hue value is rotated down');
is( $hsv[1], 100,     'too high saturation value is clamped down');
is( $hsv[2], 100,     'too high lightness value is clamped down');
