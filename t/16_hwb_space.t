...
is( ref $def->is_tuple_in_range([0, 0, 0]),     'ARRAY',   'check HWB values works on lower bound values');
is( ref $def->is_tuple_in_range([360,100,100]), 'ARRAY',   'check HWB values works on upper bound values');
is( ref $def->is_tuple_in_range([0,0]),              '',   "HWB got too few values");
is( ref $def->is_tuple_in_range([0, 0, 0, 0]),       '',   "HWB got too many values");
...
is( ref $def->is_tuple_in_range([0, -1, 0]),         '',   "whiteness value is too small");
is( ref $def->is_tuple_in_range([0, 1.1, 0]),        '',   "whiteness value is not integer");
is( ref $def->is_tuple_in_range([0, 101, 0]),        '',   "whiteness value is too big");
is( ref $def->is_tuple_in_range([0, 0, -1 ] ),       '',   "blackness value is too small");
is( ref $def->is_tuple_in_range([0, 0, 1.1] ),       '',   "blackness value is not integer");
is( ref $def->is_tuple_in_range([0, 0, 101] ),       '',   "blackness value is too big");
