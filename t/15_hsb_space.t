...
is( ref $def->range_check([0, 0, 0]),     'ARRAY',   'check HSB values works on lower bound values');
is( ref $def->range_check([360,100,100]), 'ARRAY',   'check HSB values works on upper bound values');
is( ref $def->range_check([0,0]),              '',   "HSB got too few values");
is( ref $def->range_check([0, 0, 0, 0]),       '',   "HSB got too many values");
...
is( ref $def->range_check([0, 0, -1 ] ),       '',   "brightness value is too small");
is( ref $def->range_check([0, 0, 1.1] ),       '',   "brightness value is not integer");
is( ref $def->range_check([0, 0, 101] ),       '',   "brightness value is too big");
