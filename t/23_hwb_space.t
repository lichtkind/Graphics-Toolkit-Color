...
is( $def->name,       'HWB',                     'color space has axis initials as name');
is( $def->alias,         '',                     'color space has no alias name');
is( $def->axis,           3,                     'color space has 3 axis');
is( ref $def->check_range([0, 0, 0]),     'ARRAY',   'check HWB values works on lower bound values');
is( ref $def->check_range([360,100,100]), 'ARRAY',   'check HWB values works on upper bound values');
is( ref $def->check_range([0,0]),              '',   "HWB got too few values");
is( ref $def->check_range([0, 0, 0, 0]),       '',   "HWB got too many values");
...
is( ref $def->check_range([0, -1, 0]),         '',   "whiteness value is too small");
is( ref $def->check_range([0, 1.1, 0]),        '',   "whiteness value is not integer");
is( ref $def->check_range([0, 101, 0]),        '',   "whiteness value is too big");
is( ref $def->check_range([0, 0, -1 ] ),       '',   "blackness value is too small");
is( ref $def->check_range([0, 0, 1.1] ),       '',   "blackness value is not integer");
is( ref $def->check_range([0, 0, 101] ),       '',   "blackness value is too big");
...
my $hwb = $def->convert_from( 'RGB', [ .5, .5, .5]);
...
my $rgb = $def->convert_to( 'RGB', [0, 0.5, .5]);
...
$hwb = $def->convert_from( 'RGB', [210/255, 20/255, 70/255]);
...
$rgb = $def->convert_to( 'RGB', [0.95555, 0.08, 0.18]);
...
$rgb = $def->convert_to( 'RGB', [0.83333, 0, 1]); # should become black despite color value
