...
sub okhsl_to_linear_srgb {
    my ($hsl) = @_;
    my ($h, $s, $l) = @$hsl;                   # h:0..360, s:0..1, l:0..1

    # Sonderfaelle: reines Schwarz / Weiss -> achromatisch
    return [0, 0, 0] if $l <= 0;
    return [1, 1, 1] if $l >= 1;

    $h /= 360;                                  # Grad -> 0..1 (Bjoern-intern)
    my $L = toe_inv($l);

    my $a_ = cos(2 * $PI * $h);                 # Farbtonrichtung (Einheitsvektor)
    my $b_ = sin(2 * $PI * $h);
