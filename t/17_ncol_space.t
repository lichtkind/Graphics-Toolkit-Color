...
is( ref $space->in_range([0, 0, 0]),     'ARRAY',   'check HWB values works on lower bound values');
is( ref $space->in_range([600,100,100]), 'ARRAY',   'check HWB values works on upper bound values');
is( ref $space->in_range([0,0]),              '',   "HWB got too few values");
is( ref $space->in_range([0, 0, 0, 0]),       '',   "HWB got too many values");
is( ref $space->in_range([-1, 0, 0]),         '',   "hue value is too small");
is( ref $space->in_range([1.1, 0, 0]),        '',   "hue is not integer");
is( ref $space->in_range([601, 0, 0]),        '',   "hue value is too big");
is( ref $space->in_range([0, -1, 0]),         '',   "whiteness value is too small");
is( ref $space->in_range([0, 1.1, 0]),        '',   "whiteness value is not integer");
is( ref $space->in_range([0, 101, 0]),        '',   "whiteness value is too big");
is( ref $space->in_range([0, 0, -1 ] ),       '',   "blackness value is too small");
is( ref $space->in_range([0, 0, 1.1] ),       '',   "blackness value is not integer");
is( ref $space->in_range([0, 0, 101] ),       '',   "blackness value is too big");
