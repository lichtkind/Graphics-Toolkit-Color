...
is( int @$val,                         3,  'right amount of values');
is( round_decimals( $val->[0] , 0),      1,  'L* value of black good');
is( round_decimals( $val->[1] , 0),      1,  'u* value of black good');
is( round_decimals( $val->[2] , 0),      1,  'v* value of black good');

$val = $space->normalize( [0, 0, 0] );
is( ref $val,                    'ARRAY',  'normalized tuple of zeros (black)');
is( int @$val,                         3,  'right amount of values');
is( round_decimals( $val->[0] , 0),      1,  'L value good');
is( round_decimals( $val->[1] , 0.378531073),    1,  'u* value good');
is( round_decimals( $val->[2] , 0.534351145),    1,  'v* value good');
...
is( round_decimals( $luv->[0] , 0),                1,  'first value good');
is( round_decimals( $luv->[1] , 0.378531073),      1,  'second value good');
is( round_decimals( $luv->[2] , 0.534351145),      1,  'third value good');
...
is( round_decimals( $xyz->[0] , 0),      1,  'X value good');
is( round_decimals( $xyz->[1] , 0),      1,  'Y value good');
is( round_decimals( $xyz->[2] , 0),      1,  'Z value good');
exit 0;
# white
$val = $space->denormalize( [1, .378531073, .534351145] );
is( ref $val,                    'ARRAY',  'denormalized white into zeros');
is( int @$val,                         3,  'right amount of values');
is( round_decimals( $val->[0] , 100),      1,  'L* value of white good');
is( round_decimals( $val->[1] , 0),      1,  'u* value of white good');
is( round_decimals( $val->[2] , 0),      1,  'v* value of white good');

$val = $space->normalize( [100, 0, 0] );
is( ref $val,                    'ARRAY',  'normalized tuple of white');
is( int @$val,                         3,  'right amount of values');
is( round_decimals( $val->[0] , 1),      1,  'L value good');
is( round_decimals( $val->[1] , 0.378531073),    1,  'u* value good');
is( round_decimals( $val->[2] , 0.534351145),    1,  'v* value good');

$luv = $space->convert_from( 'XYZ', [ 1, 1, 1]);
is( ref $luv,                    'ARRAY',  'deconverted white from XYZ');
is( int @$luv,                         3,  'right amount of values');
is( round_decimals( $luv->[0] , 1),                1,  'first value good');
is( round_decimals( $luv->[1] , 0.378531073),      1,  'second value good');
is( round_decimals( $luv->[2] , 0.534351145),      1,  'third value good');
...
is( int @$xyz,                         3,  'right amount of values');
is( round_decimals( $xyz->[0] , 1),      1,  'X value good');
is( round_decimals( $xyz->[1] , 1),      1,  'Y value good');
is( round_decimals( $xyz->[2] , 1),      1,  'Z value good');

# red
$val = $space->denormalize( [0.53241, .872923729, .678458015] );
is( int @$val,                          3,  'denormalize red');
is( round_decimals( $val->[0] , 53.241),  1,  'L* value of white good');
is( round_decimals( $val->[1] , 175.015), 1,  'u* value of white good');
is( round_decimals( $val->[2] , 37.756),  1,  'v* value of white good');

$val = $space->normalize( [53.241, 175.015, 37.756] );
is( int @$val,                         3,  'normalize red');
is( round_decimals( $val->[0] , 0.53241),      1,  'L value good');
is( round_decimals( $val->[1] , 0.872923729),    1,  'u* value good');
is( round_decimals( $val->[2] , 0.678458015),    1,  'v* value good');

$luv = $space->convert_from( 'XYZ', [ 0.433953728, 0.21267, 0.017753001]);
is( int @$luv,                         3,  'deconverted red from CIEXYZ');
is( round_decimals( $luv->[0] , 0.53241),                1,  'first value good');
is( round_decimals( $luv->[1] , 0.872923729),      1,  'second value good');
is( round_decimals( $luv->[2] , 0.678458015),      1,  'third value good');

$xyz = $space->convert_to( 'XYZ', [ 0.53241, .872923729, .678458015 ]);
is( int @$xyz,                         3,  'converted red to CIEXYZ');
is( round_decimals( $xyz->[0] , 0.433953728),  1,  'X value good');
is( round_decimals( $xyz->[1] , 0.21267),      1,  'Y value good');
is( round_decimals( $xyz->[2] , 0.017753001),      1,  'Z value good');

# blue
$val = $space->denormalize( [0.32297, .351963277, .036862595] );
is( int @$val,                          3,  'denormalize blue');
is( round_decimals( $val->[0] , 32.297),  1,  'L* value of white good');
is( round_decimals( $val->[1] , -9.405), 1,  'u* value of white good');
is( round_decimals( $val->[2] , -130.342),  1,  'v* value of white good');

$val = $space->normalize( [32.297, -9.405, -130.342] );
is( int @$val,                         3,  'normalize blue');
is( round_decimals( $val->[0] , 0.32297),      1,  'L value good');
is( round_decimals( $val->[1] , 0.351963277),    1,  'u* value good');
is( round_decimals( $val->[2] , 0.036862595),    1,  'v* value good');

$luv = $space->convert_from( 'XYZ', [ 0.18984292, 0.07217, 0.872771691]);
is( int @$luv,                         3,  'deconverted blue from CIEXYZ');
is( round_decimals( $luv->[0] , 0.32297),                1,  'first value good');
is( round_decimals( $luv->[1] , 0.351963277),      1,  'second value good');
is( round_decimals( $luv->[2] , 0.036862595),      1,  'third value good');

$xyz = $space->convert_to( 'XYZ', [ 0.32297, .351963277, .036862595 ]);
is( int @$xyz,                         3,  'converted blue to CIEXYZ');
is( round_decimals( $xyz->[0] , 0.18984292),  1,  'X value good');
is( round_decimals( $xyz->[1] , 0.07217),      1,  'Y value good');
is( round_decimals( $xyz->[2] , 0.872771691),      1,  'Z value good');
...
is( round_decimals( $val->[0] , 53.389), 1,  'L* value of white good');
is( round_decimals( $val->[1] , 0),      1,  'u* value of white good');
is( round_decimals( $val->[2] , 0),      1,  'v* value of white good');

$val = $space->normalize( [53.389, 0, 0] );
is( int @$val,                         3,  'normalize gray');
is( round_decimals( $val->[0] , 0.53389),        1,  'L value good');
is( round_decimals( $val->[1] , 0.378531073),    1,  'u* value good');
is( round_decimals( $val->[2] , 0.534351145),    1,  'v* value good');

$luv = $space->convert_from( 'XYZ', [ .214041474 , .21404, 0.214037086]);
is( int @$luv,                         3,  'deconverted gray from XYZ');
is( round_decimals( $luv->[0] , 0.53389),          1,  'first value good');
is( round_decimals( $luv->[1] , 0.378531073),      1,  'second value good');
is( round_decimals( $luv->[2] , 0.534351145),      1,  'third value good');

$xyz = $space->convert_to( 'XYZ', [ 0.53389, .378531073, .534351145 ]);
is( int @$xyz,                         3,  'converted gray to CIEXYZ');
is( round_decimals( $xyz->[0] , 0.214041474),   1,  'X value good');
is( round_decimals( $xyz->[1] , 0.21404),       1,  'Y value good');
is( round_decimals( $xyz->[2] , 0.214037086),   1,  'Z value good');

# nice blue
$val = $space->denormalize( [0.24082, .352573446, .317049618] );
is( int @$val,                         3,  'denormalize nice blue');
is( round_decimals( $val->[0] , 24.082), 1,  'L* value of white good');
is( round_decimals( $val->[1] , -9.189),      1,  'u* value of white good');
is( round_decimals( $val->[2] , -56.933),      1,  'v* value of white good');

$val = $space->normalize( [24.082, -9.189, -56.933] );
is( int @$val,                         3,  'normalize nice blue');
is( round_decimals( $val->[0] , 0.24082),        1,  'L value good');
is( round_decimals( $val->[1] , 0.352573446),    1,  'u* value good');
is( round_decimals( $val->[2] , 0.317049618),    1,  'v* value good');

$luv = $space->convert_from( 'XYZ', [ 0.057434743, .04125, .190608268]);
is( int @$luv,                         3,  'deconverted nice blue from CIEXYZ');
is( round_decimals( $luv->[0] , 0.24082),          1,  'first value good');
is( round_decimals( $luv->[1] , 0.352573446),      1,  'second value good');
is( round_decimals( $luv->[2] , 0.317049618),      1,  'third value good');

$xyz = $space->convert_to( 'XYZ', [ 0.24082, .352573446, .317049618 ]);
is( int @$xyz,                         3,  'converted nice blue to CIEXYZ');
is( round_decimals( $xyz->[0] , 0.057434743),   1,  'X value good');
is( round_decimals( $xyz->[1] , 0.04125),       1,  'Y value good');
is( round_decimals( $xyz->[2] , 0.190608268),   1,  'Z value good');
