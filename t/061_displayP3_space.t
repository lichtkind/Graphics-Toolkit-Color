...
my $module = 'Graphics::Toolkit::Color::Space::Instance::DisplayP3';
my $space = eval "require $module";
is( not($@), 1, 'could load the module');
is( ref $space, 'Graphics::Toolkit::Color::Space', 'got space object by loading module');
is( $space->name,                     'DISPLAYP3', 'normalized name is : "DISPLAYP3"');
...
is( $space->can_convert('Display-P3-Linear'),          1,  'do only convert from and to "Display-P3-Linear"');
is( $space->can_convert('DisplayP3Linear'),            1,  'use normalized name of converter parent');
...
my $lrgb = $space->convert_to( 'DisplayP3Linear', [0, 0, 0]);
is_tuple( $space->round($lrgb, 9), [0, 0, 0], [qw/red green blue/], "convert black back to 'Display-P3-Linear'");

$rgb = $space->convert_from( 'DisplayP3Linear', [1, 1, 1]);
is_tuple( $space->round($rgb, 9), [1, 1, 1], [qw/red green blue/], "convert white from 'Display-P3-Linear'");
$lrgb = $space->convert_to( 'DisplayP3Linear', [1, 1, 1]);
is_tuple( $space->round($lrgb, 9), [1, 1, 1], [qw/red green blue/], "convert white back to 'Display-P3-Linear'");

$rgb = $space->convert_from( 'DisplayP3Linear', [.5,.5,.5]);
is_tuple( $space->round($rgb, 9), [.735356983,.735356983,.735356983], [qw/red green blue/], "convert gray from 'Display-P3-Linear'");
$lrgb = $space->convert_to( 'DisplayP3Linear', [.735356983,.735356983,.735356983]);
is_tuple( $space->round($lrgb, 9), [.5,.5,.5], [qw/red green blue/], "convert gray back to 'Display-P3-Linear'");
